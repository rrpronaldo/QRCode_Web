...
def detect_from_video():
	...
	# lock variables
	global video_stream, outputFrame, lock, df_coupons, keys_found_set
...
def detect_text():
	if 'keyText' not in request.form:
		addLog("[ERROR] Key text not found in request.")
		return Response(status=500)
	else:
		key_text = request.form['keyText']
		if key_text == '':
			addLog('[ERROR] Key text was empty')
			return Response(status=500)
		else:
			addLog('[INFO] Starting decoding process!')
			#Extract key of NFCe from QRCode URL
			chave = re.findall(regex_chave, key_text)
			print("Chave",chave)
			#Checking if was found a valid key
			if chave:
				chave = chave[0]
				if chave not in keys_found_set:
					...
					addLog(f"[INFO] Nota {chave} já armazenada no arquivo!")
			else:
				addLog('[ERROR] Not found a valid key.')
